...
// Constants needed for analytics
...
function renderAnalytics() {
  ...
  document.getElementById('analyticsSummary').innerHTML = `
    <div class="stat-card"><div class="stat-value">${total}</div><div class="stat-label">Patients</div></div>
    <div class="stat-card"><div class="stat-value">${femalePct}%</div><div class="stat-label">Female</div></div>
    <div class="stat-card"><div class="stat-value">${labPos}</div><div class="stat-label">Lab Positives</div></div>
    <div class="stat-card"><div class="stat-value">${referred}</div><div class="stat-label">Referred</div></div>
    <div class="stat-card"><div class="stat-value">${pregnant}</div><div class="stat-label">Pregnant</div></div>
    <div class="stat-card"><div class="stat-value">${totalDispensed}</div><div class="stat-label">Meds Dispensed</div></div>`;

  document.getElementById('chartGrid').innerHTML = `
    <div class="chart-card" style="grid-column:1/-1;"><h4>Disease Burden</h4><canvas id="chartDisease" height="300"></canvas></div>
    ...
    <div class="chart-card" style="grid-column:1/-1;"><h4>Lab Positivity</h4><div id="labPositivityTable"></div></div>
    ...
    <div class="chart-card" style="grid-column:1/-1;"><h4>Referrals and Positive Labs</h4><div id="referralsList"></div></div>
    <div class="chart-card"><h4>Physician Encounters</h4><canvas id="chartProviders" height="250"></canvas></div>
    <div class="chart-card" style="grid-column:1/-1;"><h4>Medication Usage (Top 15)</h4><canvas id="chartMeds" height="300"></canvas></div>
    <div class="chart-card" style="grid-column:1/-1;"><h4>Medication Dispensing (Total Tabs/Caps)</h4><canvas id="chartDispensing" height="300"></canvas></div>`;

  drawDiseaseChart(filtered);
  drawSitesDonut(filtered);
  drawSexDonut(filtered);
  drawLabPositivity(filtered);
  drawDaysChart(filtered);
  drawAgeChart(filtered);
  drawReferralTypesChart(filtered);
  drawReferralsList(filtered);
  drawProviderChart(filtered);
  drawMedsChart(filtered);
  drawDispensingChart(filtered);
}
...
function drawSitesDonut(filtered) {
  drawDonut('chartSites', sites, sites.map(s => filtered.filter(r => r.site === s).length), ['#F68630', '#c2590a', '#fb923c', '#fed7aa', '#ea580c', '#fdba74']);
}

function drawSexDonut(filtered) {
  drawDonut('chartSex', ['Male', 'Female'], [filtered.filter(r => r.sex === 'M').length, filtered.filter(r => r.sex === 'F').length], ['#3b82f6', '#F68630']);
}

function drawLabPositivity(filtered) {
  let html = '<table style="width:100%;border-collapse:collapse;font-size:13px;"><thead><tr style="border-bottom:2px solid #e5e7eb;"><th style="text-align:left;padding:6px;">Test</th><th style="text-align:center;padding:6px;">Ordered</th><th style="text-align:center;padding:6px;">Positive</th><th style="text-align:center;padding:6px;">%</th></tr></thead><tbody>';
  LAB_TESTS.forEach(t => {
    ...
    const pctColor = pct >= 10 ? 'color:#ef4444;font-weight:700;' : pct >= 5 ? 'color:#f59e0b;font-weight:600;' : '';
    html += `<tr style="border-bottom:1px solid #f3f4f6;"><td style="padding:6px;">${t}</td><td style="text-align:center;padding:6px;">${ordered}</td><td style="text-align:center;padding:6px;">${positive}</td><td style="text-align:center;padding:6px;${pctColor}">${pct}%</td></tr>`;
  });
...
function drawReferralTypesChart(filtered) {
  const counts = {};
  filtered.forEach(r => { if (r.referralType && r.referralType !== 'None') counts[r.referralType] = (counts[r.referralType] || 0) + 1; });
...
function drawReferralsList(filtered) {
  ...
  filtered.forEach(r => {
    ...
    if (hasLabPositive(r)) {
      const posTests = LAB_TESTS.filter(t => r.labs[t] && r.labs[t].result === 'POS');
      items.push({ name: r.name || r.givenName || 'Unknown', detail: 'Lab+: ' + posTests.join(', ') });
    }
  });
  document.getElementById('referralsList').innerHTML = items.length === 0
    ? '<div style="padding:20px;text-align:center;color:#9ca3af;">No referrals or positive labs in selection.</div>'
    : '<div style="display:flex;flex-direction:column;gap:4px;">' + items.map(it =>
        `<div style="display:flex;gap:12px;padding:6px 0;border-bottom:1px solid #f3f4f6;font-size:13px;"><span style="font-weight:600;min-width:100px;">${esc(it.name)}</span><span style="color:#6b7280;">${esc(it.detail)}</span></div>`
      ).join('') + '</div>';
}

function drawProviderChart(filtered) {
  const counts = {};
  filtered.forEach(r => { if (r.provider) counts[r.provider] = (counts[r.provider] || 0) + 1; });
  const sorted = Object.entries(counts).filter(e => e[1] > 0).sort((a, b) => b[1] - a[1]);
...
function drawMedsChart(filtered) {
  ...
  filtered.forEach(r => {
    ...
    r.medications.forEach(m => {
      const name = m.name || m.medId || 'Unknown';
