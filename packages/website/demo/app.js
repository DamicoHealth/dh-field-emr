...
// ==========================================
async function init() {
  // Demo mode — skip all setup checks, go straight to app
  currentDeviceId = 'demo-device-001';
  // Hide setup wizard and device sign-in if present
  const wizard = document.getElementById('setupWizard');
  if (wizard) wizard.style.display = 'none';
  const signIn = document.getElementById('deviceSignIn');
  if (signIn) signIn.style.display = 'none';

  // Wire up demo disclaimer continue button
  const demoModal = document.getElementById('demoModal');
  const btnContinue = document.getElementById('btnDemoContinue');
  if (btnContinue && demoModal) {
    btnContinue.addEventListener('click', () => { demoModal.style.display = 'none'; });
  }
