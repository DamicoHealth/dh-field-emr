...
const pwaSync = (function() {
  ...
  async function syncNow() {
    ...
    try {
      await pushRecords();
      // Admin devices upload config changes; standard devices skip this.
  ...
  async function pushConfig() {
    ...
    for (const k of CONFIG_PUSH_KEYS) {
      try {
        ...
        items.push({ key: k, value: parsed, updated_at: new Date().toISOString() });
      } catch {}
    }
    ...
    // Avoid immediately re-pulling our own writes — bump lastConfigPull past them.
    await idbSettingSet('lastConfigPullTimestamp', new Date().toISOString());
    console.log(`[pwa-sync] Pushed ${items.length} config keys`);
  }
